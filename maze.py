...
def pid(): #simple PID-controller
    ...
    if error > last_error+15 or error > 150:
        if attempt_count > 2 and dis_f <= 600:
            moving(0.6)
            wait(10)
            print("moving_pid")
            motor_l.run_angle(600, 230, wait=False) #(speed, angle, wait)
            motor_r.run_angle(600, -230, wait=True) #the turning angle depends on your wheels, so choose it to suit yourself
            print("right turning")
            moving(1)
            print("moving_pid")
            rollback()
...
while True:
    ...
    if dis_f <= 90 and dis_l <= 150:
        ...
        wait(10)
        rollback()
    elif dis_f <= 90: #distance is in mm so it's 10 cm
        motor_r.hold()
        motor_l.hold() #hold is very strong stop
        wait(100)
        motor_l.run_angle(600, -230, wait=False) #(speed, angle, wait)
